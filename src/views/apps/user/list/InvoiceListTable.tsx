...
const InvoiceListTable = ({ invoiceData }: { invoiceData: InvoiceType[] }) => {
  ...
  const [data, setData] = useState<InvoiceType[]>(invoiceData || []) // Ensure data is always an array
  ...
  const columns = useMemo<ColumnDef<InvoiceTypeWithAction, any>[]>(
    () => [
      ...
      // Uncomment and update the `href` when localization is set up
      columnHelper.accessor('id', {
        ...
        cell: ({ row }) => (
          <Typography
            ...
            href={getLocalizedUrl(`/invoice/${row.original.id}`, locale as Locale)}
            color='primary'
            ...
            >
              <i
                className={classnames(
                  'bs-4 is-4',
                  invoiceStatusObj[row.original.invoiceStatus]?.icon || 'tabler-question-mark'
                )}
              />
            </CustomAvatar>
          </Tooltip>
  ...
  useEffect(() => {
    ...
    setData(filteredData || []) // Ensure setData receives an array
  }, [status, invoiceData])
  ...
  return (
    <Card>
        ...
        <div className='flex items-center justify-between gap-4'>
          <div className='flex items-center gap-2'>
            ...
            <CustomTextField
              ...
              className='is-[70px]'
              size='small' // Added size for better appearance
            >
      ...
      <TablePagination
        ...
        rowsPerPageOptions={[10, 25, 50]} // Ensure these match the options in the select
        labelRowsPerPage='Rows per page:' // Optional: Customize label
        // Optionally, you can integrate your custom pagination component here
        // Instead of using the default MUI TablePagination
      />
    </Card>
  )
}
