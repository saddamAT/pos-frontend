...
import { Badge, Popper, Fade, Paper, ClickAwayListener, MenuList, MenuItem, Divider, Typography } from '@mui/material'
...
export default function CompanyDropdown() {
  ...
  // console.log(userBusinesses, '--------tata')

  const selectedBusiness = user?.selectedBusiness?.name
  ...
  const handleSwitchBusiness = async (event: MouseEvent<HTMLElement> | TouchEvent, selectedBranch: any) => {
    const parentBusiness = userBusinesses.find(ub =>
      (ub.user_business || []).some((b: any) => b.id === selectedBranch.id)
    )

    if (!parentBusiness) return

    await update({ selectedBusiness: selectedBranch })
    handleDropdownClose(event)

    setSelectedBusinessForModal({
      branches: parentBusiness.user_business || [],
                ...
                <MenuList>
                  ...
                  {userBusinesses.length > 1 && [
                    ...
                    ...userBusinesses.map(ub => {
                      ...
                      const subBusinesses = Array.isArray(ub.user_business) ? ub.user_business : []
                      ...
                      return (
                        <div key={ub.id}>
                          ...
                          <Collapse in={isExpanded} timeout='auto' unmountOnExit>
                            {subBusinesses.map((sub: any) => {
                              const isSelected = session?.user?.selectedBusiness?.id === sub.id
                              return (
                                <MenuItem
                                  key={sub.id}
                                  selected={isSelected}
